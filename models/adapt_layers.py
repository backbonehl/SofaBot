...
class GCL(nn.Module):
    ...
    def predict(self, x1, a1, a2):
        emb1 = self.encoder1(x1, a1)
        emb2 = self.encoder2(x1, a2)
        prob = self.classifier(torch.cat((emb1, emb2), dim=1))
        return prob
    ...
    def forward(self, x1, a1, x2, a2):
        ...
        cl_loss = self.batch_nce_loss(proj1, proj2)

        return proj1, proj2, cl_loss

    def set_mask_knn(self, X, k, dataset, metric='cosine'):
        global global_knn
        if k != 0:
            if global_knn[dataset] is not None:
                knn = global_knn[dataset]
            else:
                ...
                global_knn[dataset] = knn
        else:
            knn = torch.eye(X.shape[0])  # 自己是自己的正样本，其他都是负样本
        self.pos_mask = knn
...
class EdgeDiscriminator(nn.Module):
    def __init__(self, input_dim, alpha, sparse, hidden_dim=128, temperature=1.0, bias=0.0 + 0.0001):
    ...
    def weight_to_adj(self, nnodes, edges, weights_lp, weights_hp):
        if not self.sparse:
            adj_lp = get_adj_from_edges(edges, weights_lp, nnodes)
            adj_lp += torch.eye(nnodes).cuda()
            adj_lp = normalize_adj(adj_lp, 'sym', self.sparse)

            adj_hp = get_adj_from_edges(edges, weights_hp, nnodes)
            adj_hp += torch.eye(nnodes).cuda()
            adj_hp = normalize_adj(adj_hp, 'sym', self.sparse)

            mask = torch.zeros(adj_lp.shape).cuda()
            mask[edges[0], edges[1]] = 1.
            mask.requires_grad = False
            adj_hp = torch.eye(nnodes).cuda() - adj_hp * mask * self.alpha
        else:
            adj_lp = dgl.graph((edges[0], edges[1]), num_nodes=nnodes, device='cuda')
            adj_lp = dgl.add_self_loop(adj_lp)
            weights_lp = torch.cat((weights_lp, torch.ones(nnodes).cuda())) + EOS
            weights_lp = norm(adj_lp, weights_lp)
            adj_lp.edata['w'] = weights_lp

            adj_hp = dgl.graph((edges[0], edges[1]), num_nodes=nnodes, device='cuda')
            adj_hp = dgl.add_self_loop(adj_hp)
            weights_hp = torch.cat((weights_hp, torch.ones(nnodes).cuda())) + EOS
    ...
    def forward(self, features, edges):
        ...
        adj_lp, adj_hp = self.weight_to_adj(nnodes, edges, weights_lp, weights_hp)
